 ...
 */
export async function _sparseCheckout(repoUrl, ref, paths) {
  ...
  const blobsIdx = await fetchObjects(
    repoUrl,
    paths.map(path => objects[path].oid)
  )
...
async function lsRefs(repoUrl, refPrefix) {
  ...
  const response = await _fetch(repoUrl + '/git-upload-pack', {
    method: 'POST',
    headers: {
  ...
  const refs = {}
  await forAwait(parseGitResponseLines(response), line => {
...
async function fetchObjects(url, objectHashes) {
  ...
  const response = await _fetch(url + '/git-upload-pack', {
    method: 'POST',
    headers: {
      Accept: 'application/x-git-upload-pack-advertisement',
      'content-type': 'application/x-git-upload-pack-request',
...
function readObject(result) {
  ...
  switch (result.type) {
    ...
    case 'tag':
      ...
      break
    default:
      throw new Error(result.oid, result.type, 'blob|commit|tag|tree')
  }
}
